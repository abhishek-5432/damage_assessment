...
# Try to import the real inference client; fall back to a local stub for dev
try:
...
# If no API key is present, do not fail hard in dev — use a stub client.
...
if _HAS_INFERENCE_SDK and InferenceHTTPClient is not None and ROBOFLOW_API_KEY:
    ...
    class _StubClient:
        ...
        def run_workflow(self, workspace_name, workflow_id, images, use_cache=True):
            # Return a deterministic dummy response matching the new array structure
            return [{
...
async def scan_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Run workflow — send image as base64 data URI (required by serverless API)
        ...
        # Handles BOTH workflow versions:
        #   OLD: { "annotated_image": {"type":"base64","value":"..."}, "predictions": {"top":"Flood","confidence":0.99,...} }
        #   NEW: { "annotated_image": "<raw_base64>", "predictions": {"image":{...},"predictions":[]}, "damage_percentage": 0 }
        result = {}
        if isinstance(workflow_result, list) and len(workflow_result) > 0:
            ...
            # Try NEW format first: check for top-level 'top' and 'confidence'
            ...
            # --- Extract damage_percentage (new workflow field) ---
            damage_pct = item.get("damage_percentage", None)
            if damage_pct is not None:
                result["damage_percentage"] = damage_pct
            
            # --- If no top/confidence from predictions, infer from inner predictions array ---
            ...
            # --- If STILL no top, try to extract from annotated_image text overlay ---
            # Some workflows embed the label in the annotated image only
            ...
            # --- Extract annotated base64 image (handles BOTH formats) ---
